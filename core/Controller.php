...
class Controller
{
    ...
    /* Récuperaire les info du site dans la base de donée  */
    public function getInfoSite()
    {
        if (!isset($_SESSION['site_info']) || empty($_SESSION['site_info'])) {
        ...
        if (!isset($_SESSION['uniqueUser'])) {
            ...
            $listVisitor = $this->Tracker->getListIP(date('Y-m-d'));

            if (!empty($listVisitor)) {

                foreach ($listVisitor as $key => $value) {

                    if ($value == $visitorIp) {

                        $_SESSION['uniqueUser'] = ['ip' => $visitorIp];
                        break;
                    }
                }
                if (!isset($_SESSION['uniqueUser'])) {

                    $this->Tracker->Addip(date('Y-m-d'), $visitorIp);
                    $_SESSION['uniqueUser'] = ['ip' => $visitorIp];
                }
            } else {
                $this->Tracker->Addip(date('Y-m-d'), $visitorIp);
    ...
    public function render($view)
    {
        ...
        /* demande la recuperation des info du site  */
        $this->getInfoSite();
